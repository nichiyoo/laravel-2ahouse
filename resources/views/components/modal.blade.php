...
<div x-data="modal" x-trap.inert.noscroll="show" x-on:open-modal.window="open" x-on:close-modal.window="close"
  x-on:keydown.escape.window="show = false" x-show="show" class="fixed inset-0 z-50 grid items-end overflow-y-auto"
  x-cloak>
  ...
  <div {{ $props }} x-show="show" x-transition:enter="ease-out duration-300"
    x-transition:enter-start="translate-y-20 translate-y-0" x-transition:enter-end="translate-y-0"
    x-transition:leave="ease-in duration-200" x-transition:leave-start="translate-y-0"
    x-transition:leave-end="translate-y-20 translate-y-0" x-on:click.outside="show = false">
    {{ $slot }}
  </div>
</div>
