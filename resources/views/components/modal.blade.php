...
<div x-data="{ show: @js($show) }" x-trap.inert.noscroll="show"
  x-on:open-modal.window="$event.detail == '{{ $name }}' ? show = true : null"
  x-on:close-modal.window="$event.detail == '{{ $name }}' ? show = false : null"
  x-on:keydown.escape.window="show = false" x-show="show"
  class="fixed inset-0 z-50 grid items-end overflow-y-auto sm:px-0" style="display: {{ $show ? 'block' : 'none' }};">
  ...
  <div {{ $props }} x-show="show" x-transition:enter="ease-out duration-300"
    ...
    x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" @click.outside="show = false">
    {{ $slot }}
  </div>
</div>
...
<script>
  document.addEventListener('alpine:init', () => {
    // Ensure the modal can be triggered via JavaScript
